...
def avoid_self_trap(board, x, y, my_size, my_body,my_id,length):
   #snakes = board["snakes"]
    ...
    if {"x": x, "y": y} in hazards or {"x": x, "y": y} in my_body or {"x": x, "y": y} in oppoenents_body:
        return False
    ...
    while queue:
        x,y = queue.pop()
        #print(x,y)
        if {"x": x, "y": y} in my_body or {"x": x, "y": y} in counted or {"x": x, "y": y} in hazards or {"x": x, "y": y} in oppoenents_body:
            continue
        else:
            #print("added")
            counted.append({"x": x, "y": y})
...
def avoid_hazards(head, board, possible_moves):
    ...
    for i in range(len(hazards)):
        if {"x": head["x"], "y": head["y"] + 1} == hazards[i]:
            try:
                possible_moves.remove("up")
            except:
                pass
        if {"x": head["x"], "y": head["y"] - 1} == hazards[i]:
            ...
            try:
                ...
                pass
        if {"x": head["x"] + 1, "y": head["y"]} == hazards[i]:
